...
# ==========================================
OUTPUT_DIR = "train_V1_transformer_66"
os.makedirs(OUTPUT_DIR, exist_ok=True)

DATA_PATH = "sliding_window_66"  # 66 維度的 ASL 訓練集
INPUT_SIZE = 66                            # 修改為 66 維             
D_MODEL = 128
NHEAD = 4
NUM_LAYERS = 2
DROPOUT = 0.3
...
def train():
    ...
    save_path = os.path.join(OUTPUT_DIR, f"transformer_66_{suffix}_best.pth")
    ...
    print(f"\n🚀 模型啟動 (ASL 66D Transformer) | 類別數: {num_classes} | Epochs: {EPOCHS}")
    ...
    # 繪製圖表
    plot_path = os.path.join(OUTPUT_DIR, f"history_transformer_66_asl_{suffix}.png")
    ...
    plt.title('Loss Curve (ASL 66D Transformer)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    # 儲存 label_map.json
    import json
    label_map = {idx: word for idx, word in enumerate(active_classes)}
    label_map_path = os.path.join(OUTPUT_DIR, "asl_label_map.json")
    with open(label_map_path, "w", encoding="utf-8") as f:
        json.dump(label_map, f, ensure_ascii=False, indent=4)
    print(f"✅ Label Map 已儲存至 {os.path.basename(label_map_path)}")

    onnx_path = os.path.join(OUTPUT_DIR, f"asl_model_{suffix}_best.onnx")
    ...
    print(f"✅ ONNX 模型已儲存至：{os.path.basename(onnx_path)}")
