"""
features_holistic_81.py
=========================
從 wlasl_tsl_subset 影片中提取 81 維特徵。

特徵組成 (81 維)：
  左手  33 維  (手腕 + 五指末端兩節共 11 點 × 3，混合歸一化)
  右手  33 維  (手腕 + 五指末端兩節共 11 點 × 3，混合歸一化)
  鼻子   3 維  (相對肩膀中心，以肩距縮放)
  左肩   3 維  (相對肩膀中心，以肩距縮放)
  右肩   3 維  (相對肩膀中心，以肩距縮放)
  左手肘 3 維  (相對肩膀中心，以肩距縮放)
  右手肘 3 維  (相對肩膀中心，以肩距縮放)
  合計 81 維

Pipeline：
  Phase 1 — HolisticLandmarker 逐幀提取原始座標 (141 維暫存)
  Phase 2 — 時序優化：空間過濾 → 插值補幀 → Savitzky-Golay 平滑
  Phase 3 — 手部關節修剪 (63->33) 與混合歸一化後輸出 .npy
"""
...
VIDEO_SOURCE        = "tsl"
DATA_PATH           = "tsl_features_81"
VIZ_PATH            = "tsl_tracking_81"
MODEL_DIR           = "models"
HOLISTIC_MODEL_PATH = os.path.join(MODEL_DIR, "holistic_landmarker.task")

# 手部骨架連線（視覺化用，僅供完整繪圖使用）
HAND_CONNS = [
    (0,1),(1,2),(2,3),(3,4),
    (0,5),(5,6),(6,7),(7,8),
...
# 66維保留的手部節點 (手腕 + 指尖兩節)
KEEP_HAND_POINTS = [0, 3, 4, 7, 8, 11, 12, 15, 16, 19, 20]
KEEP_HAND_INDICES = []
for p in KEEP_HAND_POINTS:
    KEEP_HAND_INDICES.extend([p*3, p*3+1, p*3+2])
...
# =============================================================================
# 3. 提取身體 5 點原始座標 (鼻子, 左肩, 右肩, 左手肘, 右手肘)
# =============================================================================
def get_extra_features_raw(pose_lm):
    """返回 (5, 3)：鼻子(0), 左肩(11), 右肩(12), 左手肘(13), 右手肘(14)"""
    targets = [(pose_lm, 0), (pose_lm, 11), (pose_lm, 12), (pose_lm, 13), (pose_lm, 14)]
    pts = []
...
# 4. 時序優化
#    原始資料格式 (N, 141)：
#      [左手 0:63, 右手 63:126, 鼻子 126:129, 左肩 129:132, 右肩 132:135, 左手肘 135:138, 右手肘 138:141]
# =============================================================================
...
def process_video(video_path, save_path, viz_save_path, holistic_options):
    ...
    # 暫存：左手63 + 右手63 + 鼻3 + 左肩3 + 右肩3 + 左肘3 + 右肘3 = 141 維
    raw_frames_data = []
    shoulder_params = []
    frame_idx = 0
    ...
    with HolisticLandmarker.create_from_options(holistic_options) as holistic:
        while cap.isOpened():
            ...
            # 原始座標：左手63 + 右手63 + [鼻,左肩,右肩,左肘,右肘]×3 = 141維
            ...
            extra_raw = get_extra_features_raw(pose_lm).flatten()  # (5,3)→15維
    ...
    # Phase 2：時序優化
    # ------------------------------------------------------------------
    raw_frames_data  = np.array(raw_frames_data)          # (N, 141)
    ...
    for i in range(len(refined_raw_data)):
        ...
        lsh_pt   = raw_feat[129:132]
        rsh_pt   = raw_feat[132:135]
        lelb_pt  = raw_feat[135:138]
        relb_pt  = raw_feat[138:141]
        ...
        lelb_norm = norm_pt(lelb_pt)
        relb_norm = norm_pt(relb_pt)

        # C. 節點修剪與串接 → 81 維
        # 手部修剪為各 33 維
        lh_pruned = lh_norm[KEEP_HAND_INDICES]
        rh_pruned = rh_norm[KEEP_HAND_INDICES]
        
        # 串接：左手33 + 右手33 + 鼻3 + 左肩3 + 右肩3 + 左肘3 + 右肘3 = 81維
        feat = np.concatenate([lh_pruned, rh_pruned, nose_norm, lsh_norm, rsh_norm, lelb_norm, relb_norm])
        ...
        if np.any(lelb_pt) and np.any(lsh_pt):
            cv2.circle(frame, to_px(lelb_pt), 5, (200, 100, 255), -1)
            cv2.line(frame, to_px(lsh_pt), to_px(lelb_pt), (200, 100, 255), 2)
        if np.any(relb_pt) and np.any(rsh_pt):
            cv2.circle(frame, to_px(relb_pt), 5, (200, 100, 255), -1)
            cv2.line(frame, to_px(rsh_pt), to_px(relb_pt), (200, 100, 255), 2)
...
def main():
    ...
    print("=== 開始提取 81 維特徵 (手部精簡 + 身體五點 + 混合歸一化) ===")
    print("    特徵組成：左手33 + 右手33 + 鼻3 + 左肩3 + 右肩3 + 左肘3 + 右肘3 = 81 維")
    report_lines = [
        "# 81維特徵提取偵測報告\n",
        "| 分類 | 影片名稱 | 偵測率 |",
        "| --- | --- | --- |"
    ...
    report_path = os.path.join(SCRIPT_DIR, "detection_report_holistic_81.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines) + "\n")
    print(f"\n=== 處理完成，報告已存至 {report_path} ===")
