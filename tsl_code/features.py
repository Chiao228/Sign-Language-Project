...
# --- 設定路徑 ---
VIDEO_SOURCE = "tsl"        
DATA_PATH = "tsl_features_132"   
VIZ_PATH = "tsl_tracking_132" 
MODEL_DIR = "models"
...
def get_extra_features_raw(pose_lm, face_lm):
    """ 提取原始座標：鼻子(Pose 0), 下巴(Face 152) """
    pts = []
    # 順序：鼻子, 下巴
    targets = [(pose_lm, 0), (face_lm, 152)]
...
def process_video(video_path, save_path, viz_save_path, holistic_options):
    ...
    raw_frames_data = [] # 儲存每一幀的原始座標 (132維)
    ...
    for i in range(len(refined_raw_data)):
        ...
        face_pts = raw_feat[126:132].reshape(2, 3)
        ...
        face_norm = ((face_pts - info["center"]) / info["dist"]).flatten()
        
        final_normalized_features.append(np.concatenate([lh_norm, rh_norm, face_norm]))
        ...
        for pts, color in [(lh_pts, (0, 255, 0)), (rh_pts, (0, 0, 255))]:
            if np.any(pts): # 只要有點位就繪製 (且因為內插法，中間消失的點現在也會被畫出來)
...
def main():
    ...
    print(f"=== 開始提取 132 維特徵 (純座標版本) ===")
    report_lines = ["# 132維雙重歸一化偵測報告\n", "| 分類 | 影片名稱 | 偵測率 |", "| --- | --- | --- |"]
