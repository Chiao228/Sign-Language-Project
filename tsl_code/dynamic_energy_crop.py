#!/usr/bin/env python3
"""
dynamic_energy_crop_138.py
======================
任務：動態能量裁切與濃縮 (138D 版本 - 同學的 Holistic + 雙重歸一版)
描述：此腳本將剔除開頭與結尾的靜態(發呆)幀，並將剩餘動態片段調整縮放為標準的 (30, 138) 時間軸尺寸。
"""
...
# ==========================================
SOURCE_DIR = Path("tsl_features_132")   # 剛才跑出來的 132 維資料
OUTPUT_DIR = Path("processed_npy_132")  # 裁切兼縮放完成的輸出地
...
def process_single_file(file_path, output_path):
    try:
        ...
        # 利用 OpenCV 進行時間差值拉伸或壓縮到 (30幀, 132維)
        resized = cv2.resize(cropped, (132, TARGET_FRAMES), interpolation=cv2.INTER_LINEAR)
...
def main():
    ...
    if not SOURCE_DIR.exists():
        print(f"❌ 找不到來源資料夾 (請確認 `features_holistic.py` 是否有跑完生出檔案): {SOURCE_DIR}")
        return
